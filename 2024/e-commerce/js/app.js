(() => {
    ...
    function menuInit() {
        if (document.querySelector(".icon-menu")) document.addEventListener("click", (function(e) {
            if (bodyLockStatus && e.target.closest(".icon-menu")) {
                ...
                if (document.documentElement.classList.contains("menu-open")) document.documentElement.classList.remove("search-show");
            }
        }));
    }
    function menuClose() {
    ...
    function getIsHideBanner() {
        return sessionStorage.getItem("hide-discount") ? true : false;
    }
    !function(e) {
    ...
    class QuantityProduct {
        ...
        #minAmount;
        #maxAmount;
        constructor(amount, maxAmount, minAmount = 1) {
            this.#amount = amount;
        ...
        set Amount(newAmount) {
            if (newAmount < this.#minAmount) newAmount = this.#minAmount; else if (newAmount > this.#maxAmount) newAmount = this.#maxAmount;
            this.#amount = newAmount;
            this.sendEvent();
            this.updateInterfaceAmount();
        ...
        inputAmount() {
            ...
            if (!value) return;
            const regExp = /\D/;
            if (regExp.test(value)) this.input.value = value.replace(regExp, ""); else this.Amount = parseInt(this.input.value);
        }
        inputFocusOut() {
            if (!this.input.value) this.Amount = 1;
        }
        changeAmount(typeButton) {
            if (typeButton === "plus" && this.Amount < this.#maxAmount) this.Amount++; else if (typeButton === "minus" && this.Amount > this.#minAmount) this.Amount--;
        }
        handleDisabledButton() {
            if (this.Amount === this.#minAmount) this.buttonMinus.setAttribute("disabled", ""); else this.buttonMinus.removeAttribute("disabled");
            if (this.Amount === this.#maxAmount) this.buttonPlus.setAttribute("disabled", ""); else this.buttonPlus.removeAttribute("disabled");
        }
        updateInterfaceAmount() {
            this.input.value = this.Amount;
        }
        createButton(operation = "plus") {
            const button = document.createElement("button");
            button.type = "button";
            button.className = "quantity__button";
            if (operation === "plus") button.classList.add("quantity__button--plus", "--icon-plus"); else button.classList.add("quantity__button--minus", "--icon-minus");
            return button;
        }
        createInput() {
            const wrapper = document.createElement("div");
            wrapper.className = "quantity__input";
        ...
        setEvents() {
            this.buttonPlus.addEventListener("click", this.changeAmount.bind(this, "plus"));
            this.buttonMinus.addEventListener("click", this.changeAmount.bind(this, "minus"));
            this.input.addEventListener("input", this.inputAmount.bind(this));
        ...
        render(bemClass, selectorContainer) {
            ...
            if (selectorContainer) {
                ...
                if (!container) throw new Error(`Nof found element by this selector - ${selectorContainer}`);
                container.append(wrapper);
            }
            const {wrapperInput, input} = this.createInput();
            this.input = input;
            this.buttonPlus = this.createButton();
            this.buttonMinus = this.createButton("minus");
            this.setEvents();
            this.handleDisabledButton();
            wrapper.append(this.buttonMinus);
            wrapper.append(wrapperInput);
            wrapper.append(this.buttonPlus);
            return wrapper;
        }
    }
    window.addEventListener("load", (() => {
        ...
        items.forEach((item => {
            const quantity = new QuantityProduct(1, 7);
            item.after(quantity.render());
            item.remove();
        }));
    }));
    class ProductCart {
        ...
        removeProduct() {
            const {id} = this.props;
            this.sendEvent("remove-product", {
                id
            });
            this.wrapper.remove();
        }
        changeAmount(e) {
            this.props.amount = e.detail.amount;
            const {id, amount} = this.props;
            this.sendEvent("change-amount-product", {
                id,
                amount
            });
        }
        sendEvent(type, data) {
            const event = new CustomEvent("change-product-cart", {
                detail: {
                    type,
                    data
                },
        ...
        createPreviewProduct() {
            const {srcImg, title} = this.props;
            const previewProductLink = document.createElement("a");
            previewProductLink.className = "product-cart__img";
            previewProductLink.href = this.props.link ? this.props.link : "#";
            const productImg = document.createElement("img");
            productImg.src = srcImg;
            productImg.alt = title;
            previewProductLink.append(productImg);
            return previewProductLink;
        }
        createTopBody() {
            const wrapper = document.createElement("div");
            wrapper.className = "product-cart__top";
            const title = document.createElement("h3");
            title.className = "product-cart__title";
            title.innerHTML = `<a href="${this.props.link}" target="_blank" >${this.props.title}</a>`;
            wrapper.append(title);
            const button = document.createElement("button");
            button.type = "button";
            button.className = "product-cart__remove --icon-delete";
        ...
        createCharacteristic() {
            ...
            const wrapper = document.createElement("div");
            wrapper.className = "product-cart__characteristic characteristic-product";
            wrapper.innerHTML = `\n\t\t\t<div class="characteristic-product__item">\n\t\t\t\tSize: <span>${size}</span>\n\t\t\t</div>\n\t\t\t<div class="characteristic-product__item">\n\t\t\t\tColor: <span>${color}</span>\n\t\t\t</div>\n\t\t`;
            return wrapper;
        }
        createBottomBody() {
            const {currentPrice, oldPrice, discount, amount, minAmount, maxAmount} = this.props;
            const wrapper = document.createElement("div");
            wrapper.className = "product-cart__bottom";
            const prices = document.createElement("div");
            prices.className = "product-cart__prices prices";
            prices.innerHTML = `\n\t\t\t<div class="prices__current-price">$${currentPrice.toString().padStart(2, "0")}</div>\n\t\t`;
            if (oldPrice) prices.innerHTML += `\n\t\t\t\t<div class="prices__old-price">$${oldPrice.toString().padStart(2, "0")}</div>\n\t\t\t\t<div class="prices__discount">-${discount.toString().padStart(2, "0")}%</div>\n\t\t\t`;
            wrapper.append(prices);
            const actions = document.createElement("div");
            actions.className = "product-cart__actions";
            const quantity = new QuantityProduct(amount, maxAmount, minAmount).render("product-cart");
        ...
        render(bemClass, selectorContainer) {
            ...
            wrapper.className = bemClass ? `${bemClass}__product product-cart` : "product-cart";
            wrapper.addEventListener("change-amount", this.changeAmount.bind(this));
            this.wrapper = wrapper;
            if (selectorContainer) {
                const container = document.querySelector(selectorContainer);
                if (!container) throw new Error(`Not found element by selector - ${selectorContainer}`);
                container.append(wrapper);
            }
            wrapper.append(this.createPreviewProduct());
            const contentBody = document.createElement("div");
            contentBody.className = "product-cart__body";
            contentBody.append(this.createTopBody());
            contentBody.append(this.createCharacteristic());
            contentBody.append(this.createBottomBody());
    ...
    class CartProducts {
        #totalPrice;
        constructor(productList, discountRend = 0, priceDelivery = 0) {
            if (!Array.isArray(productList)) throw new TypeError("An array of goods was expected!");
            this.productList = productList;
            this.discountRend = discountRend;
            this.priceDelivery = priceDelivery;
            this.promoCode;
            this.actualPromo = 9483;
        ...
        get TotalPriceWithAccruals() {
            let sum = this.TotalPrice;
            sum -= this.getSumOfDiscount();
            sum += this.priceDelivery;
            return sum;
        }
        getSumOfDiscount() {
            return this.TotalPrice * this.discountRend / 100;
        }
        calcTotalPrice() {
            return this.productList.reduce(((prevSum, product) => prevSum + product.currentPrice * product.amount), 0);
        }
        changeAmountProduct(id, newAmount) {
            const currentProduct = this.productList.find((item => item.id === id));
            currentProduct.amount = newAmount;
            this.TotalPrice = this.calcTotalPrice();
        }
        removeElementProductList(id) {
            this.productList = this.productList.filter((item => item.id !== id));
            this.TotalPrice = this.calcTotalPrice();
            if (!this.productList.length) this.fillProducts();
        }
        updateProductList(e) {
            const {type, data} = e.detail;
            if (type === "change-amount-product") this.changeAmountProduct(data.id, data.amount); else this.removeElementProductList(data.id);
            localStorage.setItem("cart-product", JSON.stringify(this.productList));
        }
        sendOrder(e) {
            const order = {};
            order.productList = this.productList;
            order.totalPrice = this.TotalPrice;
            if (this.discountRend) {
                order.discountRend = this.discountRend;
                order.sumOfDiscount = this.getSumOfDiscount();
                order.priceWithDiscount = this.TotalPriceWithAccruals;
            }
            console.log(order);
            e.preventDefault();
        }
        inputCode() {
            ...
            if (regExp.test(value)) this.input.value = value.replace(regExp, ""); else this.promoCode = parseInt(value);
        }
        updateOrderSummary() {
            this.fillInfoCart();
            this.totalPriceWithDiscount.textContent = `$${this.TotalPriceWithAccruals}`;
        }
        applyCode(e) {
            const button = e.currentTarget;
            if (this.promoCode === this.actualPromo) {
                this.discountRend = 20;
                button.setAttribute("disabled", "");
                this.input.setAttribute("readonly", "");
                this.updateOrderSummary();
            } else {
                this.input.classList.add("_form-error");
                const prevError = this.input.parentElement.nextElementSibling;
                if (prevError.classList.contains("error")) prevError.remove();
                this.input.parentElement.after(this.createError("Not correct promo code!"));
            }
        }
        removeWarning() {
            const prevError = this.input.parentElement.nextElementSibling;
        ...
        createInfo(message = "Cart is empty") {
            const wrapper = document.createElement("div");
        ...
        fillProducts() {
            if (!this.productList.length) {
                this.items.append(this.createInfo());
                this.wrapper.querySelector(".order-cart__submit").setAttribute("disabled", "");
                this.wrapper.querySelector(".order-cart__apply").setAttribute("disabled", "");
                this.input.setAttribute("readonly", "");
                this.input.value = "";
                return;
            }
            this.items.innerHTML = "";
            this.productList.forEach((item => this.items.append(new ProductCart(item).render("body-cart"))));
        }
        fillInfoCart() {
            this.infoCart.innerHTML = `\n\t\t\t<div class="info-order__item">\n\t\t\t\t<div class="info-order__label">Subtotal</div>\n\t\t\t\t<div class="info-order__price">$${this.TotalPrice}</div>\n\t\t\t</div>\n\t\t`;
            if (this.discountRend && this.productList.length) this.infoCart.innerHTML += `\n\t\t\t\t<div class="info-order__item">\n\t\t\t\t\t<div class="info-order__label">Discount (-${this.discountRend}%)</div>\n\t\t\t\t\t<div class="info-order__price info-order__price--c-red">-$${this.getSumOfDiscount()}</div>\n\t\t\t\t</div>\n\t\t\t`;
            this.infoCart.innerHTML += `\n\t\t\t<div class="info-order__item">\n\t\t\t\t<div class="info-order__label">Delivery ${this.priceDelivery ? "Paid" : "Free"}</div>\n\t\t\t\t${this.priceDelivery ? `<div class="info-order__price">$${this.priceDelivery}</div>` : ""}\n\t\t\t</div>\n\t\t`;
        }
        createTotalOrder() {
            const totalOrder = document.createElement("div");
            totalOrder.className = "order-cart__total total-order";
            const label = document.createElement("label");
            label.className = "total-order__label";
            label.textContent = "Total";
            totalOrder.append(label);
            const totalPrice = document.createElement("div");
            totalPrice.className = "total-order__price";
            totalPrice.textContent = `$${this.TotalPriceWithAccruals}`;
            totalOrder.append(totalPrice);
            this.totalPriceWithDiscount = totalPrice;
            return totalOrder;
        }
        createPromo() {
            const wrapper = document.createElement("div");
            wrapper.className = "order-cart__promo";
            const input = document.createElement("input");
            input.className = "order-cart__input input input--grey";
            input.type = "text";
            input.placeholder = "Add promo code";
            input.value = this.actualPromo;
            this.promoCode = this.actualPromo;
            input.setAttribute("data-no-focus-classes", "");
            input.setAttribute("maxLength", 4);
            input.addEventListener("input", this.inputCode.bind(this));
            input.addEventListener("focusin", this.removeWarning.bind(this));
            this.input = input;
            wrapper.append(input);
            const button = this.createButton("order-cart__apply", "Apply");
            button.addEventListener("click", this.applyCode.bind(this));
            wrapper.append(button);
            return wrapper;
        }
        createOrderCart() {
            const wrapper = document.createElement("div");
            wrapper.className = "body-cart__order order-cart";
            const titleElement = document.createElement("h4");
            titleElement.className = "order-cart__title";
            titleElement.textContent = "Order Summary";
            wrapper.append(titleElement);
            const infoCart = document.createElement("div");
            infoCart.className = "order-cart__info info-order";
            this.infoCart = infoCart;
            this.fillInfoCart();
            wrapper.append(infoCart);
            wrapper.append(this.createTotalOrder());
            wrapper.append(this.createPromo());
            const buttonSubmit = this.createButton("order-cart__submit", "Go to Checkout", "submit", "--icon-arrow-navigation");
            wrapper.append(buttonSubmit);
            return wrapper;
        }
        createButton(className = "", content = "", type = "button", classModify) {
            const button = document.createElement("button");
            button.className = className ? `${className} button` : "button";
            button.type = type;
            if (classModify) button.classList.add(classModify);
            button.textContent = content;
            return button;
        }
        render(className, selectorContainer) {
            const wrapper = document.createElement("form");
            wrapper.addEventListener("change-product-cart", this.updateProductList.bind(this));
            wrapper.className = className ? `${className}` : "body-cart__form";
            wrapper.setAttribute("action", "#");
            wrapper.setAttribute("method", "POST");
            wrapper.addEventListener("submit", this.sendOrder.bind(this));
            this.wrapper = wrapper;
            if (selectorContainer) {
                const container = document.querySelector(selectorContainer);
                if (!container) throw new Error(`Not found element by selector - ${selectorContainer}`);
            ...
            items.className = "body-cart__goods";
            this.items = items;
            wrapper.append(items);
            wrapper.append(this.createOrderCart());
            this.fillProducts();
            return wrapper;
        }
    }
    ...
    window.addEventListener("load", (() => {
        if (document.querySelector(".body-cart__container")) {
            let productList = JSON.parse(localStorage.getItem("cart-product"));
            if (!productList || !productList.length) {
                productList = getProductList();
                localStorage.setItem("cart-product", JSON.stringify(productList));
            }
            const cart = new CartProducts(productList);
            cart.render("", ".body-cart__container");
        }
    }));
    let gotoBlock = (targetBlock, noHeader = false, speed = 500, offsetTop = 0, isHideBanner = false) => {
        ...
        if (targetBlockElement) {
            ...
            if (noHeader) {
                ...
                if (!headerElement.classList.contains("_header-scroll")) {
                    const middleHeaderContainer = headerElement.querySelector(".main-header__container");
                    middleHeaderContainer.style.cssText = `transition-duration: 0s;`;
                    headerElement.classList.add("_header-scroll");
                    headerItemHeight = headerElement.offsetHeight;
                    headerElement.classList.remove("_header-scroll");
                    setTimeout((() => {
                        middleHeaderContainer.style.cssText = ``;
                    }), 0);
    ...
    function initSliders() {
        ...
        if (document.querySelector(".main-slider") && document.querySelector(".sub-slider")) {
            const subSlider = new swiper_core_Swiper(".sub-slider", {
    ...
    function pageNavigation() {
        ...
        if (getHash()) {
            ...
            if (goToHash) gotoBlock(goToHash, true, 500, 20, getIsHideBanner());
        }
    }
    ...
    function windowLoaded() {
        ...
        if (banner && getIsHideBanner()) hideBanner(banner);
        initMenuSpoller();
        initRating();
        ...
        document.addEventListener("formSent", (function(e) {
            ...
            if (currentForm.getAttribute("id") === "form-filter" && document.documentElement.classList.contains("filter-open")) document.documentElement.classList.remove("filter-open");
        }));
        function documentActions(e) {
            ...
            if (targetElement.closest(".banner-header__close")) {
                if (filter && document.documentElement.classList.contains("filter-open")) {
                    ...
                    const heightBannerHeader = document.querySelector(".banner-header") ? document.querySelector(".banner-header").offsetHeight : 0;
            ...
            if (targetElement.closest(".actions-review__button")) {
                ...
                if (activeButton && activeButton !== wrapper) activeButton.classList.remove("--open");
                wrapper.classList.toggle("--open");
            } else if (!targetElement.closest(".actions-review") && document.querySelector(".actions-review.--open")) document.querySelector(".actions-review.--open").classList.remove("--open");
            if (targetElement.closest(".content-catalog__filter") && filter) {
                ...
                bodyLock(300);
            } else if (targetElement.closest(".sidebar-catalog__close") || !targetElement.closest(".sidebar-catalog") && document.documentElement.classList.contains("filter-open") && !targetElement.closest(".banner-header__close")) {
            ...
            if (isTouch) {
                ...
                } else if (!targetElement.closest(".header") && document.documentElement.classList.contains("search-show")) document.documentElement.classList.remove("search-show");
                if (isTablet) if (targetElement.closest("[data-spoller-trigger]")) {
                    ...
                    if (openSubMenu && openSubMenu !== parentBlock) openSubMenu.classList.remove("--open");
                    if (parentBlock.classList.contains("--open") && document.documentElement.classList.contains("search-show")) document.documentElement.classList.remove("search-show");
                } else if (!targetElement.closest(".menu__item") && document.querySelector("[data-spoller-block].--open")) document.querySelector("[data-spoller-block].--open").classList.remove("--open");
                if (!isTablet) if (targetElement.closest("[data-spoller-trigger]")) {
            ...
            if (targetElement.closest(".tabs__title") && targetElement.closest("[data-tabs-line]")) {
                ...
                if (mediaMobileSmall.matches) calcWidthLineTabOnMobile(navigation, itemNavigation.offsetWidth);
            }
        }
    ...
    function initMenuSpoller() {
        ...
        if (mediaSpollers.length) {
            ...
            let mediaQueries = breakpoints.map((({value, typeMedia}) => `(${typeMedia}-width: ${value / 16}em),${value},${typeMedia}`));
            mediaQueries = new Set(mediaQueries);
            mediaQueries.forEach((media => {
                const options = media.split(",");
                const textMediaRequest = options[0], value = options[1], typeMedia = options[2];
                const currentSpollers = breakpoints.filter((breakpoint => breakpoint.value === value && breakpoint.typeMedia === typeMedia));
                ...
                if (mediaRequest.matches) setSpollerTabIndex(mediaRequest.matches, currentSpollers);
            }));
        }
    }
    function setSpollerTabIndex(matches, spollers) {
        spollers.forEach((({item}) => {
            ...
            matches ? toggleShowSubMenu(buttons, _slideUp, false) : toggleShowSubMenu(buttons, _slideDown);
        }));
    }
    ...
    function initRating() {
        ...
        ratingBlocks.forEach((item => {
            ...
            for (let numberStar = 0; numberStar < whole; numberStar++) body.append(getStar(ratingSize, ratingSize));
            if (remainder) {
                const width = ratingSize * remainder / .1 / 100;
                body.append(getStar(width, ratingSize));
            }
            item.append(body);
            if (item.hasAttribute("data-rating-value")) item.insertAdjacentHTML("beforeend", `\n\t\t\t\t<div class="rating__value">${value[0]}.${value[1] || "0"}/<span>5</span></div>\n\t\t\t`);
        }));
    ...
    function initMoveTabLine(matches, functionCalcWidth) {
        ...
        if (matches) functionCalcWidth(navigationWrap, activeTab.offsetWidth); else navigationWrap.style.removeProperty("--width");
    }
